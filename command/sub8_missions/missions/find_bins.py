...
def run(sub):
    ...
    yield sub.vision_proxies.colored_rectangle.set_geometry(polygon)

    # Wait for vision services, enable perception
    print_info("ACTIVATING PERCEPTION SERVICE")
    yield sub.vision_proxies.colored_rectangle.start()
    ...
    if resp is None or not resp.found:
        print_bad("BIN NOT FOUND")
        defer.returnValue(None)

    print_good("BIN POSE FOUND")
