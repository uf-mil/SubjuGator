...
def run(sub):
    ...
    point_before[2] = position[2]
    point_after[2] = position[2] 
...
def style_points(sub):
    move_spacing_left, move_step_left = np.linspace(
        0, np.pi*2, num=10, retstep=True)
    for x in move_spacing_left:
        fprint ('YAW_LEFT {}'.format(x))
        yield sub.move.yaw_left(move_step_left).zero_roll_and_pitch().go(wait=0.5)
        yield sub.nh.sleep(1)
